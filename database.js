...
db.tableordersdb = new Datastore({ filename: './database/tableorders.db', autoload: true });
...
db.diningtabledb = new Datastore({ filename: './database/diningtable.db', autoload: true });
...
app.get('/getlocal', function (req, res) {
    var productfb = req.query.getproducts
...
/////////////////////////PENDINGORDER///////////////////////////
app.post('/saveorderfb', function (req, res) {
    console.log(req.ip, req.hostname)
    req.body.status = "N"
    db.pendingordersdb.insert(req.body, function (err1, newDoc) { // Callback is optional
...
////////////////////////tableorders
app.get('/gettblorderby_id', function (req, res) {
    console.log(req.query)
    db.tableordersdb.findOne({ _id: req.query._id }, function (err1, newDoc) { // Callback is optional
        console.log(newDoc)
        res.send(newDoc)
    })
});
...
app.post('/savetblorder', function (req, res) {
    ...
    db.tableordersdb.update({ diningtablekey: req.body.diningtablekey }, req.body, { upsert: true }, function (err1, newDoc) { // Callback is optional
        db.diningtabledb.update({ TableKey: req.body.diningtablekey }, { $set: { TableStatusId: 1, UserId: req.body.UserId, UserName: req.body.UserName } }, function (err1, newDoc) {
            res.send({ msg: "success" })
        })
    })
});
...
app.get('/swaptableorders', function (req, res) {
    ...
    db.tableordersdb.update({ diningtablekey: fromtablekey }, { $set: { diningtablekey: totablekey, DiningTableId: +totablekey.split('_')[0] } }, function (err1, newDoc) { // Callback is optional
        db.diningtabledb.update({ TableKey: fromtablekey }, { $set: { TableStatusId: 0 }, $unset: { UserId: true, UserName: true } }, function (err1, newDoc) {
            db.diningtabledb.update({ TableKey: totablekey }, { $set: { TableStatusId: 1, UserId: req.body.UserId, UserName: req.body.UserName } }, function (err1, newDoc) {
                res.send({ msg: "success" })
            })
        })
    })
...
app.get('/deletetblorder', function (req, res) {
    console.log(req.query)
    db.tableordersdb.remove({ diningtablekey: req.query.diningtablekey }, { multi: true }, function (err1, newDoc) { // Callback is optional
        db.diningtabledb.update({ TableKey: req.query.diningtablekey }, { $set: { TableStatusId: 0 }, $unset: { UserId: true, UserName: true } }, function (err1, newDoc) {
            res.send({ msg: "success" })
        })
    })
});
...
app.get('/deletesplittable', function (req, res) {
    db.diningtabledb.remove({ TableKey: req.query.splittablekey }, function (err1, newDoc) { // Callback is optional
        res.send({ msg: "success" })
    })
});
...
app.post('/splitTable', function (req, res) {
    let i = 0
    db.diningtabledb.update({ TableKey: req.body.parenttable.TableKey }, req.body.parenttable, function (err1, newDoc) { // Callback is optional
        i++
        if (i == 2)
            res.send({ msg: "success" })
    })
    db.diningtabledb.insert(req.body.splittable, function (err1, newDoc) { // Callback is optional
        i++
        if (i == 2)
            res.send({ msg: "success" })
    })
...
app.get('/getordersfb', function (req, res) {
    console.log(req.ip, req.hostname)
    db.pendingordersdb.find({}, function (err1, docs) {   // Callback is optional
        res.send(docs)
    })
});
...
app.get('/deleteorderfb', function (req, res) {
    // console.log(req.query)
    db.pendingordersdb.remove({ _id: req.query._id }, function (err1, newDoc) { // Callback is optional
        res.send({ msg: "success" })
    })
});
...
app.post('/setstoredata', function (req, res) {
    ...
    db.diningtabledb.remove({}, { multi: true }, function (err, numRemoved) {
        db.diningtabledb.insert(req.body.diningTable, function (err, newDoc) {   // Callback is optional
            console.log("preference", i)
            i++
            if (i == 11)
                res.send(obj)
    ...
    db.diningareadb.remove({}, { multi: true }, function (err, numRemoved) {
        db.diningareadb.insert(req.body.diningArea, function (err, newDoc) {   // Callback is optional
            console.log("preference", i)
            i++
            if (i == 11)
                res.send(obj)
